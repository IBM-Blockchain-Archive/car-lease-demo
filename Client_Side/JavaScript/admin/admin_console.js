...
function showEditTbl()
{
    ...
    let house_number = randomNumber(1, 150);

    let street_name = addresses.street_names[randomNumber(0, addresses.street_names.length-1)];

    let city_int = randomNumber(0, addresses.cities.length-1);
    ...
    let city = addresses.cities[city_int];
    let post_code = addresses.post_codes[city_int]+(randomNumber(1, 99))+' '+(randomNumber(1, 9))+text;

    let name = names[randomNumber(0, names.length)];

    let types = [name+'\'s Motors', name+'\'s Car Sales', name+'\'s Leasing', name, name+'\'s Scrap Merchants'];
...
{
    let output = Math.floor(Math.random() * max) + min;
    console.log(output);
    return output;
}
