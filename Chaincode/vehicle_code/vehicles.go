...
func (t *SimpleChaincode) check_affiliation(stub shim.ChaincodeStubInterface, cert string) (int, error) {
	...
	if err != nil {
		return -1, errors.New("Couldn't decode certificate: " + err.Error())
	}

	av, err := attr.GetValueFrom("affiliation", bytes)

	if err != nil {
		return -1, errors.New("Couldn't read value: " + err.Error())
	}

	affiliation, _ := strconv.Atoi(string(av))
...
func (t *SimpleChaincode) update_colour(stub shim.ChaincodeStubInterface, v Vehicle, caller string, caller_affiliation int, new_value string) ([]byte, error) {
	...
	if 	v.Owner	== caller &&
		caller_affiliation == MANUFACTURER && (
			(v.Owner == caller && caller_affiliation == MANUFACTURER) ||
			caller_affiliation == AUTHORITY) &&
		v.Scrapped == false {
