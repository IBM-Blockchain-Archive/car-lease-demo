...
let create = function(req,res)
{
    ...
    {
        cars_info = initial_vehicles[scenario];
    }
    else
    {
        let error = {};
...
function create_cars(req, res)
{
    ...
    send_error = false;
    let prevCount = -1;
    let check_create = setInterval(function(){
...
function create_car()
{
    ...
    let url = configFile.config.app_url + '/blockchain/assets/vehicles';
    ...
    request(options, function(error, response, body)
    {
        ...
        if (!error && response.statusCode == 200 && body.indexOf('error') == -1)
        {
            ...
            {
                let data = JSON.parse(array[i]).message;
...
function transfer_created_cars(req, res)
{
    ...
    send_error = false;
    let prevCount = -1;
    let check_int = setInterval(function(){
...
function transfer_car(sender, receiver, id, function_name, toUpdate)
{
    ...
    let str = 'user='+sender;
    let cookie = request.cookie(str);
    let url = configFile.config.app_url + '/blockchain/assets/vehicles/'+id+'/owner';
    j.setCookie(cookie, url);
    let options = {
        url: url,
        json: data,
...
function update_cars(req, res)
{
    send_error = false;
    let prevCount = -1;
    let check_update = setInterval(function(){
        if(counter == cars.length && !send_error)
        {
...
let ind_update_counter = 0;

function update_all_car_parts(id)
{
    let car_owner = cars[counter].Owners[1];
    let update_fields = [{'value':cars[counter].VIN,'field':'VIN', 'title': 'VIN'},{'value':cars[counter].Make,'field':'make', 'title': 'Make'},{'value':cars[counter].Model,'field':'model', 'title': 'Model'},{'value':cars[counter].Colour,'field':'colour', 'title': 'Colour'},{'value':cars[counter].Reg,'field':'reg', 'title': 'Registration'}];
    let prevCount = -1;
    let check_ind_update = setInterval(function(){
...
function update_car(manufacturer, value, id, field)
{
    ...
    let j = request.jar();
    let str = 'user='+manufacturer;
    let cookie = request.cookie(str);
    let url = configFile.config.app_url + '/blockchain/assets/vehicles/'+id+'/'+field;
    j.setCookie(cookie, url);
    let options = {
        url: url,
        json: data,
...
function transfer_updated_cars(req, res)
{
    send_error = false;
    let prevCount = -1;
    let check_trans = setInterval(function(){
        if(counter == cars.length && !send_error)
        {
...
let ind_transfer_counter = 2;

function transfer_all_owners(id)
{
    let prevCount = -1;
    let check_ind_transfer = setInterval(function(){
        ...
        {
            let types = ['manufacturer_to_private', 'private_to_lease_company', 'lease_company_to_private', 'private_to_scrap_merchant'];
...
{
    let demo_status_file = fs.readFileSync(__dirname+'/../../../logs/demo_status.log');
    let demo_status = JSON.parse(demo_status_file);
