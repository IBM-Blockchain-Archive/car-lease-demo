        ...
        chain.getUser(user_id, function(err, user) {
			...
			res.end(JSON.stringify(error))
                        ...
                        var invokeRequest = {
                                ...
                                fcn: "update_make", // function_name.toString(),
                                args: [newValue, v5cID],
                                attrs: attrList
                        }
                        ...
                        invokeTx.on('submitted', function(results) {
                                tracing.create('INFO', 'PUT blockchain/assets/vehicles/vehicle/'+v5cID+'/make', 'Request to update make submitted succesfully');
                        });
                        ...
                        invokeTx.on('error', function(error) {
                                ...
                                var error = {}
                                error.error  = true;
                                error.message = 'Unable to confirm make update.';
                                tracing.create('ERROR', 'PUT blockchain/assets/vehicles/vehicle/'+v5cID+'/make', error)
                                res.end(JSON.stringify(error))
                        });
                }
